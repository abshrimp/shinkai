...
class Tilemap {
	...
	constructor( img, size ) {
		...
		//人を置くタイルを指定できる
		this.human = [];
		this.quest = 'quest1    セントビンセント及びグレナディーン諸島に行け';
	} //constructor() 終了
	 ...
	 * 引数
	 * mapX : タイルマップ上のX座標
	 * mapY : タイルマップ上のY座標                         ここに会話入れる！！！！
	 */
	hasObstacle( mapX, mapY ) {
		//指定された場所のタイルが、壁や天井など、移動できないかどうか
		//mapYは頭の座標
		const _isObstacleTile = this.obstacles.some( obstacle => obstacle === this.data[mapY + 1][mapX] );
		//移動できないかどうかを返す
	...
	hasHuman( mapX, mapY ) {
		...
		for(var i=0; i<this.human.length; i++){
			if(this.human[i] === this.data[mapY + 1][mapX]) return this.human[i];
		}
		return -1;
	} //hasObstacle() 終了
	...
	update( stage, canvas ) {
		...
		for ( let i=0; i<this.tiles.length; i++ ) {
			...
			this.tiles[i].update( stage, canvas );
		...
		this.render( stage, canvas, false );
	...
	render( stage, canvas, first ) {
		...
		for (let y=0; y<this.data.length; y++) {
			...
			for (let x=0; x<this.data[y].length; x++) {
				if(!first && this.data2[y][x] == -1) continue;
				//タイルの横の位置
				...
				if(first){
					//X方向に、何番目の画像か
					var _frameX = this.data[y][x] % ( this.img.width / this.size );
					//Y方向に、何番目の画像か
					var _frameY = Math.floor( this.data[y][x] / ( this.img.width / this.size ) );
				}else{
					//X方向に、何番目の画像か
					var _frameX = this.data2[y][x] % ( this.img.width / this.size );
					//Y方向に、何番目の画像か
					var _frameY = Math.floor( this.data2[y][x] / ( this.img.width / this.size ) );
				}

				var bmp = new createjs.Bitmap( this.img );
				bmp.sourceRect = new createjs.Rectangle(_frameX * this.size, _frameY * this.size, this.size, this.size);
				bmp.x = _tileX;
				bmp.y = _tileY;
				stage.addChild( bmp );
			}
